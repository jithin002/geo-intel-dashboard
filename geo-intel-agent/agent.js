...
const ADVANCED_MASK = "places.id,places.displayName,places.location,places.rating,places.userRatingCount,places.priceLevel,places.types,places.formattedAddress,places.businessStatus";
...
const DOMAIN_TYPES = {
  ...
  restaurant: {
    competitors: ["restaurant", "cafe"],
    // matches frontend: competitorTypes
  ...
  retail: {
    competitors: ["supermarket", "department_store", "convenience_store"],
    // matches frontend
...
function isInsideBangalore(lat, lng) {
  return lat >= BANGALORE_BOUNDS.south && lat <= BANGALORE_BOUNDS.north && lng >= BANGALORE_BOUNDS.west && lng <= BANGALORE_BOUNDS.east;
}
...
async function callAnalyzeLocation(lat, lng, radius, domainId) {
...
function buildReport(location, domain, intel, scores) {
  const domainCfg = DOMAIN_TYPES[domain] || DOMAIN_TYPES["gym"];
  const competitors = intel.competitors.places || [];
  const highRated = competitors.filter((p) => (p.rating || 0) >= 4);
  const avgRating = intel.competitors.averageRating;
  const levelLabel = { LOW: "Low", MEDIUM: "Medium", HIGH: "High", VERY_HIGH: "Very High" };
  const gapLabel = { UNTAPPED: "Untapped", OPPORTUNITY: "Opportunity", COMPETITIVE: "Competitive", SATURATED: "Saturated" };
...
NOTABLE COMPETITORS:
`;
    competitors.slice(0, 5).forEach((p) => {
...
const analyzeLocation = new FunctionTool({
  ...
  description: 'Performs a full location intelligence analysis for a named area in Bangalore. Returns competitor count, demand drivers, transit access, site score, and a strategic recommendation. Use this when the user asks about a specific area, wants to know if a location is good for their business, or when answering questions about specific attributes of a location (e.g. "nearest premium gym", "least competitive cafe area"). Only supports these domains: gym, restaurant, cafe, retail, bank, coworking. If the user asks for an unsupported domain, still call this tool \u2014 it will return a graceful message.',
  parameters: z.object({
  ...
  execute: async ({ location, domain, radius_meters }) => {
    ...
    console.log(`\u{1F4CD} Geocoded "${location}" \u2192 [${lat}, ${lng}]`);
...
const compareLocations = new FunctionTool({
  ...
  description: 'Compares two Bangalore areas side-by-side for a specific business domain. Returns scores, key metrics, and a clear winner recommendation. Use this when the user asks "which is better", "compare X vs Y", or "X or Y for my business". Only supports: gym, restaurant, cafe, retail, bank, coworking.',
  parameters: z.object({
    location1: z.string().describe("First Bangalore area to compare."),
    location2: z.string().describe("Second Bangalore area to compare."),
    domain: z.enum(["gym", "restaurant", "cafe", "retail", "bank", "coworking"]).describe("Business domain / type being evaluated.").default("gym"),
    radius_meters: z.number().optional().default(1e3)
  }),
  execute: async ({ location1, location2, domain, radius_meters }) => {
    ...
    if (!DOMAIN_TYPES[domain]) {
      return {
        status: "not_mapped",
        domain,
        message: `The "${domain}" domain is not supported for map-based comparison. Supported domains are: gym, restaurant, cafe, retail, bank, and co-working spaces.`
      };
    }
    ...
    return {
      ...
      comparison: {
        ...
        winnerScore,
        margin,
        recommendation: margin < 5 ? `Both areas are closely matched (${scores1.total} vs ${scores2.total}). Consider practical factors like rent and accessibility.` : `${winner} leads by ${margin} points (${winnerScore}/100). It offers a meaningfully better opportunity for a ${DOMAIN_TYPES[domain]?.label || domain}.`
      }
    };
  }
});
const searchNearby = new FunctionTool({
  name: "search_nearby",
  description: 'Searches for specific place types near a Bangalore location. Use this when the user wants to find specific businesses \u2014 e.g. "show me gyms near MG Road", "coffee shops in Whitefield", or "coworking spaces in HSR".',
  parameters: z.object({
    location: z.string().describe("Bangalore area or landmark to search near."),
    place_type: z.string().describe(
  ...
  execute: async ({ location, place_type, radius_meters }) => {
    ...
    const formatted = places.slice(0, 10).map((p) => ({
...
const rootAgent = new LlmAgent({
  ...
  description: "Geo-Intel Assistant \u2014 analyzes optimal business locations in Bangalore, India using real-time Places API data.",
